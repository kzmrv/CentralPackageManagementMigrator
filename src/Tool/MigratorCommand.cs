...
namespace Tool
{
    internal sealed class MigratorCommand : AsyncCommand<MigratorCommand.Settings>
    {
        ...
        private (Dictionary<string, NuGetPackageInfo> latestPackageByName, Dictionary<FileInfo, Dictionary<string, NuGetPackageInfo>> packageInfoByProjectFile) ParseProjectFiles(List<FileInfo> allProjectFiles, Settings settings)
        {
            ...
            {
                var packages = ParseProjectFile(settings, projectFile, latestPackage);
                if (packages != null)
                {
        ...
        private static Dictionary<string, NuGetPackageInfo>? ParseProjectFile(Settings settings, FileInfo projectFile, Dictionary<string, NuGetPackageInfo> latestPackage)
        {
